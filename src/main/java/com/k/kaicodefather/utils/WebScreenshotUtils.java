...
public class WebScreenshotUtils {
    ...
    /**
     * 线程隔离的WebDriver（移除static块初始化，避免线程绑定错误）
      */

    private static final ThreadLocal<WebDriver> driverThreadLocal = new ThreadLocal<>();

    /**
     * 获取当前线程的Chrome驱动（懒加载初始化）
     */
    ...
    public static String saveWebPageScreenshot(String webUrl) {
        ...
        try {
            ...
            // 关键：释放驱动资源 + 清理ThreadLocal
            if (webDriver != null) {
                try {
                    webDriver.quit(); // 关闭浏览器进程，避免内存泄露
                } catch (Exception e) {
                    log.warn("关闭Chrome驱动失败", e);
                }
            }
            driverThreadLocal.remove(); // 清理线程变量，避免内存泄露
        }
    }
